  ...
  const renderBookingCard = (booking) => {
    ...
    return (
      <div key={booking._id} className="bg-[#111f11] border border-[#1f3a1f] rounded-2xl p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-white font-bold">{booking.tractorId?.tractorName || 'Tractor'}</h3>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${statusColors[booking.status] || 'bg-yellow-950 text-yellow-300'}`}>
                  {booking.status}
                </span>
              </div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${typeColors[booking.serviceType] || 'bg-green-900/50 text-green-300'}`}>
        ...
        <div className="flex items-center gap-4 text-xs text-[#4b6b4b] mb-4 flex-wrap">
          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {booking.hours} hours</span>
            ...
            {!loadingB && bookings.length > 0 && (
              <div className="space-y-8">
                ...
                {bookings.filter(b => b.status === 'pending').length > 0 && (
                  <div>
                    <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                      <span className="w-2.5 h-2.5 bg-yellow-500 rounded-full animate-pulse" /> Pending Bookings
                    </h2>
                    <div className="space-y-4">
                      {bookings.filter(b => b.status === 'pending').map(booking => renderBookingCard(booking))}
                    </div>
                  </div>
                )}
                ...
                {bookings.filter(b => b.status === 'accepted').length > 0 && (
                  <div>
                    <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                      <span className="w-2.5 h-2.5 bg-green-500 rounded-full" /> Accepted Bookings
                    </h2>
                    <div className="space-y-4">
                      {bookings.filter(b => b.status === 'accepted').map(booking => renderBookingCard(booking))}
                    </div>
                  </div>
                )}

                {/* If there are bookings but none are pending or accepted */}
                {bookings.filter(b => b.status === 'pending').length === 0 && bookings.filter(b => b.status === 'accepted').length === 0 && (
                  <div className="text-center py-24">
                    <CheckCircle2 className="w-16 h-16 text-[#2d4a2d] mx-auto mb-4" />
