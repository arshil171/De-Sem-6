...
const BookTractor = () => {
  ...
  const [form, setForm]               = useState({ hours: 1, serviceType: '', farmLocation: '' })
  const [fieldErrors, setFieldErrors] = useState({})

  const serviceTypes = ['Ploughing', 'Harvesting', 'Seeding', 'Spraying']
  const BASE = import.meta.env.VITE_BASE_URL

  useEffect(() => { fetchTractor() }, [id])
  ...
  const validate = () => {
    ...
    if (tractor && form.hours > (tractor.maxHoursPerBooking || 8))
      errs.hours = `Maximum ${tractor.maxHoursPerBooking || 8} hours`
    setFieldErrors(errs)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Booking confirmed! Driver has been notified via email.')
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-[#111f11] border border-[#1f3a1f] rounded-2xl p-6 mb-5">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-green-900 text-green-300 shrink-0">
              {tractor.tractorType}
            </span>
          </div>
          <div className="flex items-center gap-3 mt-4 bg-[#0d1a0d] rounded-xl px-4 py-3">
            <div className="w-8 h-8 rounded-full bg-yellow-700 flex items-center justify-center text-[#0a150a] text-xs font-extrabold shrink-0">
              {tractor.driverId?.name?.charAt(0).toUpperCase()}
            </div>
            <div>
              <p className="text-green-300 text-sm font-semibold">{tractor.driverId?.name}</p>
              <p className="text-[#4b6b4b] text-xs">Verified Driver</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#111f11] border border-[#1f3a1f] rounded-2xl p-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="text-[#a3c4a3] text-xs font-bold uppercase tracking-widest mb-3 block">Service Type</label>
              <div className="grid grid-cols-2 gap-2">
                {serviceTypes.map(type => (
                  <label key={type} className="cursor-pointer">
                    <input type="radio" name="serviceType" value={type}
            ...
            <div>
              <label className="text-[#a3c4a3] text-xs font-bold uppercase tracking-widest mb-2 block">Farm Location</label>
            ...
            <div>
              <label className="text-[#a3c4a3] text-xs font-bold uppercase tracking-widest mb-3 block">Hours Needed</label>
              <div className="flex items-center gap-4 bg-[#0a150a] border-2 border-[#2d4a2d] rounded-xl px-5 py-4">
                <button type="button"
                  onClick={() => setForm(f => ({ ...f, hours: Math.max(1, f.hours - 1) }))}
                  className="w-10 h-10 rounded-xl bg-[#1f3a1f] hover:bg-yellow-900/40 text-white font-bold text-xl flex items-center justify-center transition-colors"
                >−</button>
                <div className="flex-1 text-center">
                  <span className="text-yellow-400 text-4xl font-black">{form.hours}</span>
                  <span className="text-[#4b6b4b] text-sm ml-2">hrs</span>
                </div>
                <button type="button"
                  onClick={() => setForm(f => ({ ...f, hours: Math.min(tractor.maxHoursPerBooking || 8, f.hours + 1) }))}
                  className="w-10 h-10 rounded-xl bg-[#1f3a1f] hover:bg-yellow-900/40 text-white font-bold text-xl flex items-center justify-center transition-colors"
                >+</button>
              </div>
              <div className="flex justify-between mt-1.5">
                <p className="text-[#4b6b4b] text-xs flex items-center gap-1">
                  <Clock className="w-3 h-3" /> Max: {tractor.maxHoursPerBooking || 8} hours
                </p>
                {fieldErrors.hours && <p className="text-red-500 text-xs">{fieldErrors.hours}</p>}
              </div>
            </div>

            <div className="bg-[#0d1a0d] border border-[#1f3a1f] rounded-xl p-5">
              <div className="flex justify-between text-sm text-green-600 mb-2">
                <span>Rate</span>
                <span>₹{tractor.pricePerHour} × {form.hours} hr{form.hours > 1 ? 's' : ''}</span>
              </div>
              <div className="flex justify-between items-center border-t border-[#1f3a1f] pt-3">
            ...
            <button type="submit" disabled={submitting}
              className="w-full py-4 bg-yellow-600 hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed text-[#0a150a] font-extrabold text-sm rounded-xl flex items-center justify-center gap-2 transition-colors"
            >
              {submitting
